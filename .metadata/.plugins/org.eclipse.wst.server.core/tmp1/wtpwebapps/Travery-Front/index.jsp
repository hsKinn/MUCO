...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#btnJoin").click(function() {
			...
			var userPassword = $("#SignUpuserPassword").val();
			userPassword = $.trim(userPassword);
			if (userPassword == "") {
				alert("비밀번호를 입력하세요!");
			...
			var userPassword = $("#SignUpuserPasswordCheck").val();
			userPassword = $.trim(userPassword);
			if (userPassword == "") {
				alert("비밀번호를 재입력하세요!");
				$("#SignUpuserPasswordCheck").focus();''
				return;
			}
		...
		$("#btnLogin").click(function() {
			...
			var userEmail = $("#LoginPassword").val();
			if (userEmail == "") {
				alert("비밀번호를 입력하세요!");
				$("#LoginPassword").focus();
				return;
			...
			//로그인 비밀번호 확인 
			$.post("<c:url value="/loginPasswordCheck" />", {
				"password" : $("#LoginPassword").val() , "email" : $("#LoginUserEmail").val()
			}, function(data) {
				var jsonData3 = {};
				try {
				...
				if (jsonData3.result) {
					...
					if (jsonData3.isCorrectLoginInfo) {
		...
		//비밀번호 6자리 체크 
		$("#SignUpuserPassword").blur(function() {
			var password = $("#SignUpuserPassword").val();
			if (password.length < 6) {
				$("#SignUpuserPassword").val("");
				$("#SignUpPasswordCheck").show();
			} 
			else {
				$("#SignUpPasswordCheck").hide();
			}
		...
		//비밀번호 재입력 체크 
		$("#SignUpuserPasswordCheck").blur(function() {
			var password = $("#SignUpuserPassword").val();
			var passwordCheck = $("#SignUpuserPasswordCheck").val();
			if (password != passwordCheck) {
				$("#SignUpuserPasswordCheck").val("");
				$("#SignUpReEnterPasswordCheck").show();
			} 
			else {
				$("#SignUpReEnterPasswordCheck").hide();
			}
		...
		$("#SignUpUserName").blur(function() {
         $.post("<c:url value="/nameCheck" />", {
            ...
            if (jsonData2.result) {
               if (jsonData2.isExistName || $("#SignUpUserName").val() == "") {
                  $("#SignUpNickNameCheck").show();
                  $("#SignUpUserName").val("");
                  $("#submit").attr("disabled", true);
               } else {
                  $("#SignUpNickNameCheck").hide();
...
<header id="first">
	<div class="header-content">
		<div class="inner">
			...
			<h4>Searching fit travel place with awesome Travery searching
				service</h4>
			<hr>
	...
	<div class="container">
		<div class="row">
			<div
				...
				<br>
				<p class="text-faded">
					Does not have Travery account? Join us!<br /> If have, Enjoy
					Travery!
				</p>
				<a id="letsJoin" class="btn btn-default btn-xl page-scroll">Sign
					Up</a> &nbsp; <a id="letsLogIn"
					class="btn btn-default btn-xl page-scroll">Login</a>
				<div class="clear"></div>
				<div id="index">
					...
					<div id="join" style="margin: auto;">
						<form id="joinForm">
							...
							<input type="text" class="form-control" tabindex="4" id="SignUpUserName" name="SignUpUserName" placeholder="User name">
								
								 <br />
							<div class="alert alert-info" id="SignUpEmailCheck">
								<strong>Email !</strong> You have already Travery account. <br />
								<a href="#" style="color: #333333;"><strong>Forgot
										your password? come here!</strong></a>
							</div>
							<div class="alert alert-info" id="SignUpNickNameCheck">
								<strong>User name !</strong> This nick name is already taken.
								Please choose another nick name.
							</div>
							...
							<div class="alert alert-info" id="SignUpReEnterPasswordCheck">
								<strong>Password !</strong> Please enter your password
								correctly.
							</div>
							...
							<label> <a class="btn btn-primary btn-xl page-scroll" id="letsJoinBtn">Let's
									Join!</a> <input type="button" id="btnJoin" tabindex="5"
								value="Signup" style="display: none;" />
							</label>
						</form>
					</div>
					...
					<div id="login" style="margin: auto;">
						<form id="loginForm">
							<input type="text" class="form-control" tabindex="1"
								id="LoginUserEmail" name="LoginUserEmail" placeholder=" Email">
							<input type="password" class="form-control" tabindex="2"
								id="LoginPassword" name="LoginPassword" placeholder=" Password"> <br />
							<div class="alert alert-info" id="LoginEmailCheck">
								<strong>Upps !</strong> Please enter your email/password correctly. <br />
								<a href="#" style="color: #333333;"><strong>Don't
										you have Travey account? Join us!</strong></a>
							</div>
							<label> <a class="btn btn-primary btn-xl page-scroll">Let's
									fun!</a> <input type="button" id="btnLogin" tabindex="3"
	...
	<div class="container">
		<div class="row">
			<div class="col-lg-4 col-md-4 text-center">
				<div class="feature">
					<i class="icon-lg glyphicon glyphicon-phone wow fadeIn"
						data-wow-delay=".3s"></i>
					<h3>Travery SNS</h3>
			...
			<div class="col-lg-4 col-md-4 text-center">
				<div class="feature">
					<i class="icon-lg glyphicon glyphicon-globe wow fadeInUp"
						data-wow-delay=".2s"></i>
					<h3>Search</h3>
					<p class="text-muted">Easy to search travel place with
						visualization searching tool</p>
				</div>
			</div>
			<div class="col-lg-4 col-md-4 text-center">
				<div class="feature">
					<i class="icon-lg glyphicon glyphicon-calendar wow fadeIn"
						data-wow-delay=".3s"></i>
					<h3>Reservation</h3>
	...
	<div class="container-fluid">
		<div class="row no-gutter">
			<div class="col-lg-4 col-sm-6">
				<a class="gallery-box" data-toggle="modal"
